...
## __[ 2021.12.08]__ <br></br>
>- __합성과 상속, 리액트는 강력한 합성 모델을 가지고 있다. 상속 대신 합성을 사용하여 코드를 재사용 하는것이 좋다.__

>- __Key는 일반적으로 항목이 고정적일 경우, 배열의 인덱스 값을 사용한다. 다만 항목의 순서가 바뀔 수 있는 경우 인덱스 사용을 권장하지 않으며, 이것때문에 성능이 저하되거나 컴포넌트의 STATE와 관련된 문제가 발생할 수 있다.__


## __[ 2021.11.24]__ <br></br>
>- __오늘은 이론 위주의 공부를 했다. 리액트의 경우, 처음부터 점진적으로 적용 가능하도록 설계가 되었으며, 필요한 만큼만 리액트를 사용할 수 있다. 온라인 코드 편집기를 사용하여 간편한 경험도 가능하다. 리액트의 경우, 자바 스크립트를 어느정도 알고 있는 상태에서 공부 시 더 효율이 좋으며, 이해가 가능할 것이다.__


## __[ 2021.11.17]__ <br></br>

>- __review__
>- __상태를 가지는 컴포넌트, this.props를 이용해 입력 데이터를 다루는 것 외에도 내부적인 데이터를 가질 수 있다. 이는 이전에 사용했떤 코드였던 this.state로 접근 가능하다.__
>- __애플리케이션, props와 state를 사용해서 간단한 todo 리스트를 만들 수 있다. state를 사용해 사용자가 입력한 정보들을 관리한다.__
>- __markdown-editor을 만들었으며, 해당 코드는 깃에 업로드 되어있다.__


## __[ 2021.11.10]__ <br></br>
